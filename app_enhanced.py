...
from pandas.api.types import is_datetime_64_any_dtype
import warnings
...
st.set_page_config(
    ...
    page_icon="https://i.imgur.com/azwWWQN.png",  # Using logo as page icon
    layout="wide",
...
<style>
    ...
    .main-header {
        ...
        margin-bottom: 2rem;
        padding: 1rem;
        background: linear-gradient(90deg, var(--miva-ash) 0%, #ffffff 50%, var(--miva-ash) 100%);
        border-radius: 10px;
    ...
    .status-error {
        background: var(--miva-red);
    }

    .info-box {
        background: var(--miva-ash);
        padding: 1rem;
        border-radius: 8px;
        border-left: 4px solid var(--miva-blue);
        margin: 1rem 0;
    }
...
class Visualizer:
    ...
    def plot_missing_values(df: pd.DataFrame, title: str):
        ...
        fig = px.bar(
            ...
            color_continuous_scale='Reds' # Red is appropriate for missing values
        )
    ...
    def plot_time_trends(df: pd.DataFrame, title: str, freq: str = "D"):
        ...
        time_cols = [c for c in df.columns if is_datetime_64_any_dtype(df[c])]
        if not time_cols:
...
def create_dashboard():
    ...
    with st.sidebar:
        # Add Miva Logo
        st.image("https://i.imgur.com/azwWWQN.png", use_column_width=True)
